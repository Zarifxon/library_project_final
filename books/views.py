...
# Bittagina obektni o'zini ko'rsatuvchi view
...
class BookListApiView(APIView):
    def get(self, request):
        ...
        data = {
            "status": f"Retuned {len(books)} books",
...
class BookDeleteApiView(APIView):
    def delete(self, request, pk):
        try:
            book=Book.objects.get(id=pk) #get_object_or_404
            book.delete()
            return Response({
                "status": True,
...
class BookUpdateApiView(APIView):
    ...
    def put(self, request, pk):
        ...
        if serializer.is_valid(raise_exception=True):
            ...
            return  Response({
                "status": True,
                "message" : f"Book {book_saved} update successful"
